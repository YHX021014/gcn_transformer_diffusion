...
class Visualizer:
    ...
    def visualize(self,
                  ...
                  viz_time_step=None):
        if viz_type == 'bbox':
            self.viz_bbox_trajectories(inputs, normalized=normalized, bbox_type=bbox_type, color=color,
                                       viz_time_step=viz_time_step)
        elif viz_type == 'point':
    ...
    def viz_distribution(self, dist, id_to_show, radius=3, thickness=3):
        ...
        for i, (pi, mu, cov) in enumerate(zip(pis, mus, covariances)):
            ...
            if self.mode == 'plot':
                ...
                cv2.drawMarker(self.img, tuple(mu.astype(np.int32)), color=colors[i], markerType=0, markerSize=radius,
                               thickness=thickness)
        if self.mode == 'plot':
            pi_stats = 'Max pi:{:.3f},  Min pi:{:.3f} \nMean pi:{:.3f}, STD pi:{:.3f}'.format(pis.max(), pis.min(),
                                                                                              pis.mean(), pis.std())
    ...
    def viz_bbox_trajectories(self, bboxes, normalized=False, bbox_type='x1y1x2y2', color=None, thickness=4, radius=5,
                              viz_time_step=None):
        '''
        bboxes: (T,4) or (T, K, 4)
        '''
