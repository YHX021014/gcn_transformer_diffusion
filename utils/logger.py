...
class Logger(logging.Logger):
    ...
    def __init__(self, name, project, viz_backend=None, sync=True):
        super().__init__(name)
        ...
        if viz_backend is not None and not self.disabled:
            assert viz_backend in BACKENDS, "Please specify either None or a backend in {}".format(
                BACKENDS)
            self._create_backend(project, viz_backend, sync)
    ...
    def _create_backend(self, project, backend, sync):
        if backend == 'wandb':
            self.backend = _WandBBackend(project, sync)
        elif backend == 'tensorboardx':
            self.backend = _TensorboardXBackend(project, sync)
...
class _Backend:
    ...
    def __init__(self, project, run_id, log_dir, sync=True):
...
class _WandBBackend(_Backend):
    ...
    def __init__(self, project, sync=True):

        os.environ['WANDB_MODE'] = 'run' if sync else 'dryrun'
        wandb.init(project=project, entity="morning")
        super().__init__(project, wandb.run.id, wandb.run.dir, sync=sync)
...
class _TensorboardXBackend(_Backend):
    ...
    def __init__(self, project, sync=False):
        HASH_LENGTH = 8
        ...
        super().__init__(project, run_id, log_dir, sync=sync)
